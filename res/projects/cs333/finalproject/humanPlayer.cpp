...
bool HumanPlayer::checkLegal(Game& game, int& x, int& y, ostream& out)
{
    ...
    int dblCheck = (tempSpot+gridSpot +game.getMap().getCol() )/2;
    //out << tempSpot <<endl;
...
bool HumanPlayer::checkLegalComp(Game& game, int x, int y)
{
    bool moveClear = true;
    int tempSpot;
    int yVar = 0;
...
int  HumanPlayer::possibleMove(Game& game, int x, int y)
{
...
int HumanPlayer::testX(int spot, Game& game)
{
