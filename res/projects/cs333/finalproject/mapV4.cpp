...
void Map::correctMap( int location)
{


 /// Setup left data
    //cout << "Before first if statement \n\n\n";
    if (myMap[location-1] != 11 && myMap[location-1]  != -12 && myMap[location-1] != -13  && myMap[location-1] != -1 && myMap[location-1] != -1 != 0 )
    {
       myMap[location] = myMap[location-1]+1;
        //cout << "The data of spot: " << location - 1 << " is now equal to: " << myMap[location-1] << endl;
    }
        //cout << "Before second if statement \n\n\n";
     if (myMap[location+1] != -11 && myMap[location+1] != -12 && myMap[location+1] != -13  && myMap[location+1] != -1 && myMap[location+1] != 0)
    {
       myMap[location] = myMap[location+1]+1;
        //cout << "The data of spot: " << location - 1 << " is now equal to: " << myMap[location-1] << endl;
    }
        //cout << "Before third if statement \n\n\n";
    if (myMap[columns+location-1] != -11 && myMap[columns+location-1] != -12 && myMap[columns+location-1] != -13 && myMap[columns+location-1] != -1
    && myMap[columns+location-1] != 0 )
    {
        myMap[location] = myMap[columns+location-1]+1;
        //cout << "The data of spot: " << columns+location-1 << " is now equal to: " <<  myMap[columns+location-1] << endl;
    }
    ...
    /// Upleft Diagnol
    if (myMap[location-1-columns] != -11 && myMap[location-1-columns] != -12 && myMap[location-1-columns] != -13 && myMap[location-1-columns] != -1
    && myMap[location-1-columns] != 0)
    {

        myMap[location] = myMap[location-1-columns]+1;
    }
    ...
    /// Down data setup
    if (myMap[columns+location] != -11 && myMap[columns+location] != -12 && myMap[columns+location] != -13 && myMap[columns+location] != -1
    && myMap[columns+location] != 0)
    {
        myMap[location] = myMap[columns+location];
    }

    /// Up
    if (myMap[location-columns] != -11 && myMap[location-columns] != -12 && myMap[location-columns] != -13 && myMap[location-columns] != -1
    && myMap[location-columns] != 0)
    {
        myMap[location] = myMap[location-columns];
    }
}
...
int Map::dataSpot(int spot)
{
    return myMap[spot];
}

void Map::placeCar(int location, int carNum)
{
    int carVar =0;
    if (carNum == 1)
    carVar = -11;
    ...
    myMap[location] = carVar;
}

void Map::changeData(int spot,int data)
{
    myMap[spot] = data;
}
